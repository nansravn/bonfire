...
| P6 | The timer's interval follows `BONFIRE_IDLE_CHECK_INTERVAL`: `bonfire-update.sh` writes a drop-in `bonfire-agent.timer.d/interval.conf`. The committed unit says 1 min. The drop-in restates `OnBootSec=30s` because the reset line clears all monotonic timers. | agent/systemd |
