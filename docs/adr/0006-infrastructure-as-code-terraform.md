...
- **Every cloud-init change replaces the VM.** Three replacements happened on 2026-09-12, about five minutes each; the data disk, its world and the public IP survived every one, as the design intended. Replacement is the normal path for configuration changes, not an incident, but it needs an empty server.
- **Two things were done from the CLI and must be brought back:** the owner's Cosmos Data Reader role ([ADR 0005](0005-storage-table-cosmos-keyvault-blob.md) amendment) and `terraform untaint` of the Function app after its failed publish. Neither is drift Terraform would repair; the first is a one-line `iam` addition.
